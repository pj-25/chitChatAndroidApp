...
public class MessageDecoder implements DataConsumer {
    ...
    public MessageDecoder(MessageConsumer msgConsumer){
        this.msgConsumer = msgConsumer;
    }

    public MessageDecoder(MessageConsumer msgConsumer, ServerChannel serverChannel){
        this.msgConsumer = msgConsumer;
        ...
        for(String msg:dataStrings){
            ...
            switch (msgCode[0]){
                case "-1":  //offline friend <- msg send
                    msgConsumer.consume("Internet", msgCode[1]+" is offline :(", Gravity.CENTER+"");
                    break;
                ...
                case "0":   //message from friend
                    msgConsumer.consume(msgCode[1], msgCode[2], Gravity.LEFT+"");
                    break;
